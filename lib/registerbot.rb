...
# The Class handling the incoming messages by users from the Telegram Bot-API. Runs a listen loop that regularly fetches all updates in chats with the bot.
class Registerbot

    # Creates a new `Registerbot` instance. Uses the Database given to create two seperate tables for reports and feedback. Creates one instance of each `Reports_processor`, `Helper_processor` and `Feedback_processor` to process incoming messages of each type.
    #
    ...
    def initialize(bot_token, db, image_path)
        # initialize a logger
        ...
        # Start the actual `Telegram::Bot::Client` which listens to user messages
        Telegram::Bot::Client::run(bot_token) do |reg_bot|
            @bot = reg_bot
        end
        download_url = "https://api.telegram.org/file/bot#{bot_token}"
        @reports_processor = Reports_processor.new(@bot, reports, image_path, download_url)
        @feedback_processor = Feedback_processor.new(@bot, feedback)
